package com.kevinpratama.mathopia.gamescene.pengurangan;
...
public class PenguranganGameScene10 extends BaseScene {
...
//	private Number[] numberHolder;
	...
	private final int STAGE_ID = 30;
	private int mysteryBoxCount = 1;
	private final int REWARD_ID = 5;
	...
	public void onBackKeyPressed() {
		...
		// oceanMusic.stop();
		// applauseSound.stop();
		SceneManager.getInstance().loadPenguranganStageMenuSceneFromGameScene(
				engine);
	}
	...
	private void createSound() {
		...
		jungleMusic = resourcesManager.jungleMusic;
	...
	private void createBackground() {
		...
		attachChild(new Sprite(640, 400,
				resourcesManager.pengurangan_game_scene_background_region, vbom) {
			@Override
...
//		numberHolder = new Number[12];
		swappableNumbers = new SwappableNumber[12];
		mathSymbols = new MathSymbol[3];
		choicesHolder = new Sprite[11];
		...
		switch (random) {
		case 0:
			numbers = new int[] { 2543, 1123, 524, 3142, 3124, 3214, 3241,
					3412, 3421, 4421, 4321, 4213 };
			break;
		case 1:
			numbers = new int[] { 2816, 1004, 324, 3496, 3469, 3649, 3694,
					3964, 3946, 4946, 4964, 4496 };

			break;
		case 2:
			numbers = new int[] { 2759, 1328, 253, 3834, 3384, 3348, 3843,
					3483, 3438, 4438, 4834, 4843 };
			break;
		case 3:
			numbers = new int[] { 2846, 1523, 475, 3894, 3849, 3984, 3948,
					3498, 3489, 4894, 4849, 4884 };
			break;
		case 4:
			numbers = new int[] { 3470, 2130, 538, 5062, 5026, 5206, 5260,
					5620, 5602, 4602, 4062, 4002 };
			break;
...
//		}
		...
		Sprite[] answerHolder = new Sprite[3];
		answerHolder[0] = this.answerHolder[2];
		answerHolder[1] = this.answerHolder[4];
		answerHolder[2] = this.answerHolder[6];

		Sprite[] mathSymbolsHolder = new Sprite[3];
		mathSymbolsHolder[0] = this.answerHolder[3];
		mathSymbolsHolder[1] = this.answerHolder[5];
		mathSymbolsHolder[2] = this.answerHolder[7];

		Sprite[] detachHolder = new Sprite[4];
		detachHolder[0] = this.answerHolder[0];
		detachHolder[1] = this.answerHolder[1];
		detachHolder[2] = this.answerHolder[9];
		detachHolder[3] = this.answerHolder[10];

		question = new int[] { 0, 1, 2 };
		indexChoicesHolder = new int[] { 1, 2, 3, 4, 5, 6, 7, 8, 9 };
		indexChoicesNumber = new int[] { 3, 4, 5, 6, 7, 8, 9, 10, 11 };
	...
	private boolean checkAnswer() {
		...
		int userAnswer = -1;
		for (int i = 0; i < swappableNumbers.length; i++) {
			if (this.answerHolder[8].collidesWith(swappableNumbers[i])) {
				userAnswer = swappableNumbers[i].getNumber();
				break;
			}
		}

		if (userAnswer == numbers[3]) {
			false_sign.setVisible(false);
			eagle[0].detachSelf();
			eagle[1].detachSelf();
			eagle[2].detachSelf();
		...
		// if (GameManager.checkPengurangan(numberHolder, answerHolder,
		// userAnswer)) {
		// false_sign.setVisible(false);
		// eagle[0].detachSelf();
		// eagle[1].detachSelf();
	...
	private class menuItemListener implements IOnMenuItemClickListener {
		...
		public boolean onMenuItemClicked(MenuScene pMenuScene,
			...
			case MENU_STAGE_RESET:
				SceneManager.getInstance().resetPenguranganGameScene(engine,
						STAGE_ID);
				return true;
			default:
				return false;
