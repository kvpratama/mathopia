package com.kevinpratama.mathopia.gamescene.perkalian;
...
public class PerkalianGameScene2 extends BaseScene{
...
//	private final int answer = 6531;
	...
	public void onBackKeyPressed() {
		...
		// oceanMusic.stop();
		// applauseSound.stop();
		SceneManager.getInstance().loadPerkalianStageMenuSceneFromGameScene(
				engine);
	}
	...
	private void createSound() {
		...
		lakeMusic = resourcesManager.lakeMusic;
	...
	private void createBackground() {
		...
		attachChild(new Sprite(640, 400,
				resourcesManager.perkalian_game_scene_background_region, vbom) {
			@Override
...
//		numberHolder = new Number[7];
		swappableNumbers = new SwappableNumber[7];
		...
		mathSymbols[0] = new MathSymbol(0, 0,
				resourcesManager.game_scene_number_holder_region, vbom, "x");
		mathSymbols[1] = new MathSymbol(0, 0,
				resourcesManager.game_scene_number_holder_region, vbom, "=");

		
		int[] numbers = { 26, 25, 3, 4, 104, 144, 124 };
...
//		detachHolder[4] = this.answerHolder[9];
		...
		Sprite problem = new Sprite(0, 0, resourcesManager.perkalian_problem_region, vbom);
		problem.setPosition(this.answerHolder[2]);
		attachChild(problem);
	...
	private boolean checkAnswer() {
		...
		answerHolder[0] = this.answerHolder[5];
		answerHolder[1] = this.answerHolder[7];
		answerHolder[2] = this.answerHolder[9];
//		answerHolder[3] = this.answerHolder[7];
		
//		int userAnswer = -1;
//		for (int i = 0; i < numberHolder.length; i++) {
//			if(this.answerHolder[7].collidesWith(numberHolder[i])){
...
//		}
		...
		for (int i = 0; i < userAnswers.length; i++) {
			for (int j = 0; j < swappableNumbers.length; j++) {
//				if(i == 0){
					if(answerHolder[i].collidesWith(swappableNumbers[j])){
						userAnswers[i] = swappableNumbers[j].getNumber();
						break;
					}
//				}
			}
...
//		if (GameManager.checkPengurangan(numberHolder, answerHolder, userAnswer)) {
//			false_sign.setVisible(false);
//			eagle[0].detachSelf();
//			eagle[1].detachSelf();
//			eagle[2].detachSelf();
	...
	private class menuItemListener implements IOnMenuItemClickListener {
		...
		public boolean onMenuItemClicked(MenuScene pMenuScene,
			...
			case MENU_STAGE_RESET:
				SceneManager.getInstance().resetPerkalianGameScene(engine,
						STAGE_ID);
				return true;
			default:
				return false;
