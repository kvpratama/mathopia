...
public class PerkalianGameScene10 extends BaseScene {
...
//	private Number[] numberHolder;
	...
	private final int STAGE_ID = 40;
	private int mysteryBoxCount = 1;
	private final int REWARD_ID = 8;
...
//		numberHolder = new Number[10];
		...
		switch (random) {
		case 0:
			numbers = new int[] { 18, 6, 9, 12, 21, 30, 11, 31, 32, 22 };
			break;
		case 1:
			numbers = new int[] { 22, 3, 6, 11, 12, 21, 41, 31, 33, 22 };
			break;
		case 2:
			numbers = new int[] { 24, 6, 9, 16, 26, 36, 10, 6, 13, 11 };
			break;
		case 3:
			numbers = new int[] { 20, 5, 4, 25, 21, 30, 31, 35, 15, 5 };
			break;
		case 4:
			numbers = new int[] { 17, 4, 2, 34, 43, 30, 14, 24, 10, 22 };
			break;
...
//		detachHolder[3] = this.answerHolder[8];
		...
		indexChoicesNumber = new int[] { 3, 4, 5, 6, 7, 8, 9 };
	...
	private boolean checkAnswer() {
		...
		if (GameManager.checkPerkalian(swappableNumbers, answerHolder, userAnswer)) {
			false_sign.setVisible(false);
			eagle[0].detachSelf();
			eagle[1].detachSelf();
			eagle[2].detachSelf();
