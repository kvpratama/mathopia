...
public class PembagianGameScene2 extends BaseScene{
	private Sprite[] choicesHolder, answerHolder;
	private SwappableNumber[] s;
...
//	private Eagle[] eagles;
	...
	private final int STAGE_ID = 42;
	private int mysteryBoxCount = 0;
	private final int REWARD_ID = 0;
	// private final int answer = 6531;
...
//		numberHolder = new Sprite[5];
		s = new SwappableNumber[5];
		mathSymbols = new MathSymbol[0];
		choicesHolder = new Sprite[11];
		...
		for (int i = 0; i < choicesHolder.length; i++) {
			choicesHolder[i] = new Sprite(choicesHolderX, 520,
					resourcesManager.pembagian_holder_region, vbom);
			choicesHolderX += 100;
		}
		...
		// int[] numbers = { 2543, 1123, 1420, 1240, 1204, 1402, 2402, 2420,
		// 1422 };
		
		for (int i = 0; i < s.length; i++) {
			if(i == 4){
				s[i] = new SwappableNumber(0, 0, resourcesManager.pembagian_problem_region, vbom, s, answerHolder, choicesHolder);
				s[i].setSwappable(false);
			}else{
				s[i] = new SwappableNumber(0, 0, resourcesManager.pembagian_choices_region[i], vbom, s, answerHolder, choicesHolder);
			}
		}

//		for (int i = 0; i < numberHolder.length; i++) {
//			if (i == 4) {
//				numberHolder[i] = new Sprite(0, 0,
//						resourcesManager.pembagian_problem_region, vbom) {
...
//			} else {
//				numberHolder[i] = new Sprite(0, 0,
//						resourcesManager.pembagian_choices_region[i], vbom) {
//					boolean mGrabbed = false;
//
//					@Override
//					public boolean onAreaTouched(TouchEvent pSceneTouchEvent,
...
//		}
		...
		Sprite[] mathSymbolsHolder = new Sprite[0];
		// mathSymbolsHolder[0] = this.answerHolder[4];
		// mathSymbolsHolder[1] = this.answerHolder[6];

		Sprite[] detachHolder = new Sprite[9];
		detachHolder[0] = this.answerHolder[0];
		detachHolder[1] = this.answerHolder[1];
		detachHolder[2] = this.answerHolder[2];
		detachHolder[3] = this.answerHolder[4];
		detachHolder[4] = this.answerHolder[5];
		detachHolder[5] = this.answerHolder[6];
		// detachHolder[5] = this.answerHolder[7];
		detachHolder[6] = this.answerHolder[8];
		detachHolder[7] = this.answerHolder[9];
		detachHolder[8] = this.answerHolder[10];

		question = new int[] { 4 };
		indexChoicesHolder = new int[] { 1, 4, 7, 10 };
		indexChoicesNumber = new int[] { 0, 1, 2, 3 };
		...
		GameManager.setPenjumlahanQuestion4(question, s,
				answerHolder, mathSymbols, mathSymbolsHolder, detachHolder,
				this);
		GameManager.setPembagianChoices(indexChoicesNumber, indexChoicesHolder,
				s, choicesHolder, indexChoicesPosition);
...
//		answerHolder[1] = this.answerHolder[5];
		...
		for (int i = 0; i < s.length; i++) {
			if (this.answerHolder[7].collidesWith(s[0])) {
				// userAnswer = numberHolder[i].getNumber();
				userAnswer = true;
				break;
			}
		}
...
//	private void checkAnswer(Sprite numberHolder){
//		if(this.s[0] == numberHolder){
//			false_sign.setVisible(false);
//			eagle[0].detachSelf();
//			eagle[1].detachSelf();
//			eagle[2].detachSelf();
...
//	}
	...
	private void startGame() {
		...
		for (int i = 0; i < moveXEagle.length; i++) {
			moveXEagle[i] = new MoveXModifier(30, eagleXOrigin,
					turtle[i].getX());
		}
		
//		eagles = new Eagle[3];
//		
//		eagles[0] = new Eagle(eagleXOrigin, 340, resourcesManager.eagle_region, vbom, turtle[0], 3);
//		eagles[1] = new Eagle(eagleXOrigin, 340, resourcesManager.eagle_region, vbom, turtle[1], 3);
//		eagles[2] = new Eagle(eagleXOrigin, 340, resourcesManager.eagle_region, vbom, turtle[2], 3, true);
//		
//		eagles[0].setNextEagle(eagles[1]);
//		eagles[1].setNextEagle(eagles[2]);
//		
//		eagles[0].startMove();
...
//			// TODO Auto-generated method stub
//			if (turtleCount == 0 && gameEnd == false) {
//				gameEnd = true;
//				onGameComplete(turtleCount, false);
//			}
//			
//			if(eagles[0].collidesWith(turtle[0])){
//				eagles[0].setPosition(0, 600);
//				turtleCount--;
//				eagles[1].startMove();
//			}
//			if(eagles[1].collidesWith(turtle[1])){
//				eagles[1].setPosition(0, 600);
//				turtleCount--;
//				eagles[2].startMove();
//			}
//			if(eagles[2].collidesWith(turtle[2])){
//				eagles[2].setPosition(0, 600);
//				turtleCount--;
//				eagles[0].uneag
//				eagles[1].startMove();
//			}
