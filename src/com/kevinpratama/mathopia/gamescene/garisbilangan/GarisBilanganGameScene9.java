...
public class GarisBilanganGameScene9 extends BaseScene {
...
//		ITextureRegion[] region = new ITextureRegion[13];
...
//		number = new Sprite[13];
//		numberHolder = new Number[13];
		swappableNumbers = new SwappableNumber[13];
		eagleXOrigin = 1280;
		...
		int[] numbers = {330, 331, 332, 333, 334, 335, 336, 337, 338, 339, 340, 329, 341};
		...
		for (int i = 0; i < swappableNumbers.length; i++) {
			swappableNumbers[i] = new SwappableNumber(0, 0,
					...
					swappableNumbers, answerHolder, choicesHolder,
					numbers[i]);
		}
		...
		GameManager.setGarisBilanganQuestion(question, swappableNumbers, answerHolder,
				this);
		GameManager
				.setChoices(indexChoicesNumber,
						indexChoicesHolder, swappableNumbers, choicesHolder,
						indexChoicesPosition);
		
//		Number number = new Number(100, 600, resourcesManager.number_holder_region, vbom, 888);
//		attachChild(number);
//		Number number2 = new Number(100, 500, resourcesManager.number_holder_region, vbom, 8888);
//		attachChild(number2);
	...
	private boolean checkAnswer() {
		if (GameManager.checkGarisBilangan(swappableNumbers, answerHolder)) {
			false_sign.setVisible(false);
			eagle[0].detachSelf();
			eagle[1].detachSelf();
			eagle[2].detachSelf();
	...
	private void onGameComplete(int turtleCount, boolean winLose) {
		...
		GameManager.onGameCompleteDetachChild(menuChildScene, turtle, swappableNumbers,
				mystery_box, this);
		...
		public boolean onMenuItemClicked(MenuScene pMenuScene,
			...
			case MENU_RESET:
				GameManager.resetChoicesPosition(
						indexChoicesNumber, swappableNumbers, choicesHolder,
						indexChoicesPosition);
				beepSound.play();
				return true;
			case MENU_COMPLETE:
