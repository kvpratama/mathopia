...
public class PenjumlahanGameScene5 extends BaseScene {
...
//		numberHolder = new Number[11];
		swappableNumbers = new SwappableNumber[11];
...
//		int[] numbers = { 6531, 6000, 4000, 500, 700, 30, 80, 1, 7, 40 };
		int[] numbers = { 2000, 400, 90, 3, 2493, 2439, 2934, 2943, 2394, 2349, 2593 };
...
//		}
		...
		answerHolder[1] = this.answerHolder[3];
		...
		question = new int[] { 0, 1, 2, 3 };
		indexChoicesHolder = new int[] { 2, 3, 4, 5, 6, 7, 8 };
		indexChoicesNumber = new int[] { 4, 5, 6, 7, 8, 9, 10 };
	...
	private boolean checkAnswer() {
		...
		int userAnswer = -1;
		for (int i = 0; i < swappableNumbers.length; i++) {
			if(this.answerHolder[9].collidesWith(swappableNumbers[i])){
				userAnswer = swappableNumbers[i].getNumber();
				break;
			}
		}
		
		if (GameManager.checkPenjumlahan(swappableNumbers, answerHolder, userAnswer)) {
			false_sign.setVisible(false);
			eagle[0].detachSelf();
			eagle[1].detachSelf();
			eagle[2].detachSelf();
